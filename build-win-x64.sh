#!/usr/bin/env bash
# ═══════════════════════════════════════════════════════════════════
#  QMark — Build for Windows x86_64 (cross-compile from Linux)
...
#    Qt 6     — Built from source with -static for mingw64
#    SQLite3  — Amalgamation compiled with mingw64-gcc
#    libsodium — ./configure --host=x86_64-w64-mingw32 --disable-shared --enable-static
...
#    2. Project-local: ./deps/qt6-win64, ./deps/sqlite3, ./deps/libsodium-win64
#    3. User home: ~/qt6-win64, ~/sqlite3, ~/libsodium-win64
#    4. System: /opt/qt6-win64, /opt/sqlite3, /opt/libsodium-win64
#
#  To build dependencies first:  ./build-deps.sh win-x64
# ═══════════════════════════════════════════════════════════════════
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
SRC_DIR="$SCRIPT_DIR/src"
BUILD_DIR="$SCRIPT_DIR/build-win-x64"
OUTPUT="$SCRIPT_DIR/QMark-x64.exe"
...
# ── Path detection ────────────────────────────────────────────────
# Searches in order: env var → project deps/ → ~/deps → /opt
find_dir() {
    for d in "$@"; do
        [ -d "$d" ] && echo "$d" && return
    done
...
SODIUM_DIR="${SODIUM_DIR:-$(find_dir \
    "$SCRIPT_DIR/deps/libsodium-win64" \
    "$HOME/libsodium-win64" \
    "$HOME/deps/libsodium-win64" \
    /opt/libsodium-win64)}"
...
if [ -z "$QT_DIR" ]; then
    ...
    echo "  Or place files in ./deps/qt6-win64/" >&2
    missing=1
fi
...
if [ -z "$SODIUM_DIR" ]; then
    ...
    echo "  Or place files in ./deps/libsodium-win64/" >&2
    missing=1
fi
...
# ── Common flags ──────────────────────────────────────────────────
CXXFLAGS="-std=c++17 -O2 -Wall -DWIN32 -DUNICODE -D_UNICODE -DMINGW_HAS_SECURE_API=1 -DQT_STATICPLUGIN \
  -I$QT_DIR/include \
...
# Generate and compile moc output
